...
-- 2 insight: our business showed weird data in 2016, perhaps there was something wrong with deliveries of our products. 
-- now we see that growth wasn't as rapid over the years - it's just that there's data for only 3 months of 2016.
...
-- 3 next question: did users have enough time to order again? let's check retention rates
with customer_order_count as 
(
select 
customer_unique_id,
case 
	when customer_order_count = 0 then '0'
...
-- however, this metric is influenced by observation window bias, as newer cohorts have had less time to return.  
...
(
select 
distinct customer_unique_id, customer_id, min(order_purchase_timestamp) over (partition by customer_unique_id) as start_date, order_purchase_timestamp
from olist_orders_dataset ood
